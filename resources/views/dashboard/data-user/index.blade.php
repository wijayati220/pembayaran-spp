			...
			<div class="card">
				<div class="card-body">
					<div class="card-title">Data User</div>
                              <a href="{{ url('dashboard/data-user/create') }}" class="btn btn-success btn-rounded float-right mb-3">
                                 <i class="mdi mdi-plus-circle"></i> {{ __('Tambah user') }}
                              </a>
                                            ...
                                            <td>										                           
                               	 		  <div class="hide-menu">
                                    			<a href="javascript:void(0)" class="text-dark" id="actiondd" role="button" data-toggle="dropdown">
                                        			...
                                        			<a class="dropdown-item" href="{{ url('dashboard/data-user/'. $value->id .'/edit') }}"><i class="ti-pencil"></i> Edit </a>
										     @if(auth()->user()->id != $value->id)
                                                        <form method="post" action="{{ url('dashboard/data-user', $value->id) }}" id="delete{{ $value->id }}">
												@csrf
												@method('delete')
